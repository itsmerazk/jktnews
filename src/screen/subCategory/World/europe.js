...
export default function Europe() {
    ...
    useEffect(()=> {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://jakpost.vercel.app/api/category/world/europe');
                const data = response.data;
                setFeaturedPost(data.featured_posts);
                setPosts(data.posts);
    ...
    return (
        ...
        <div className="container px-4 py-8 mx-auto">
          <div className="mb-8">
            ...
            <p className="mb-2 text-gray-700">{featuredPost.pusblised_at}</p>
            <a href={featuredPost.link} className="text-blue-500">Read More</a>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((post, index) => (
              <div key={index} className="p-4 bg-gray-100 rounded-lg shadow-md">
                ...
                <p className="mb-2 text-gray-700">{post.pusblised_at}</p>
